...
@SearchIndexable
class FortuneCraft : SettingsPreferenceFragment(), Preference.OnPreferenceChangeListener, Indexable {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val enabled = Settings.System.getInt(resolver, NETWORK_TRAFFIC_STATE, 0) == 1
        mNetTrafficState?.isChecked = enabled
    ...
    private fun updateNetTrafficSummary() {
        val enabled = Settings.System.getInt(
            requireActivity().contentResolver, NETWORK_TRAFFIC_STATE, 0
        ) == 1
        updateNetTrafficSummary(enabled)
    }

    private fun updateNetTrafficSummary(enabled: Boolean) {
    ...
    private fun updateNetTrafficValue() {
        if (mNetTrafficState == null) return
        val enabled = Settings.System.getInt(
            requireActivity().contentResolver, NETWORK_TRAFFIC_STATE, 0
        ) == 1
