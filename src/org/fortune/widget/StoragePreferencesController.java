...
import org.fortune.widget.StoragePreference;
...
public class StoragePreferencesController extends BasePreferenceController {
  ...
  public void updateState(Preference preference) {
    if (preference != null) {
      final PrivateStorageInfo info =
          PrivateStorageInfo.getPrivateStorageInfo(getStorageManagerVolumeProvider());
      final long totalBytes = info.totalBytes;

      BidiFormatter instance = BidiFormatter.getInstance();
      String totalSummary =
          instance.unicodeWrap(Formatter.formatShortFileSize(mContext, totalBytes));
      String usedSummary =
          Formatter.formatShortFileSize(mContext, info.totalBytes - info.freeBytes);
      mPreference.setProgress(
          (int) ((((double) (totalBytes - info.freeBytes)) / ((double) totalBytes)) * 100.0d));
